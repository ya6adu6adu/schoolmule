...
function mainContentFullScreen() {
	if (he) {
		...
		$("#first-menu").hide();
		$("#header").hide();
		$("#hide-navigation").toggleClass("expanded");
...
function eventMenuClick(x) {
	var i;
	var items=document.getElementById("event_calendar_menu").childNodes[1].childNodes;
	for (i in items) items[i].className="item-inactive";
	x.className="item-active";
	
	if ($(x).text() == 'Reoccuring event') {
		$('#calendar_wrap').hide();
		$('#reoccuring_wrap').show();
	}
	
	if ($(x).text() == 'Recipients') {
		$('#reoccuring_wrap').hide();
		$('#calendar_wrap').show();	
	}
}
...
function setLeftTab(index) {
	...
	function enableDateInput() {
		var selectId, inputId, option;
		selectId = 'reoccur-end';
		option = document.getElementById(selectId).getElementsByTagName("option");
		inputId = 'reoccur-enddate';
		if (option[1].selected) {
			$('#end-date-disabled').hide();
			$('#end-date-enabled').show();
		}
		else {
			$('#end-date-enabled').hide();
			$('#end-date-disabled').show();
		}
	}

document.write('<style type="text/css">select.styled { position: relative; opacity: 0; filter: alpha(opacity=0); z-index: auto; } </style>');
...
var Custom = {
	...
	// Reoccuring event dialog - Setting select spans width after opening
	dialogInit: function() {
		var inputs = document.getElementsByTagName("select");
		for(a = 0; a < inputs.length; a++) {
			if(inputs[a].name.slice(0,7) == "reoccur") {
				var spanId = 'select'+inputs[a].name.slice(0,20);
				document.getElementById(spanId).style.width = inputs[a].offsetWidth - 23 + "px";
			}
		}
	},
	// Bulletin editor show (calendar-add-event.html) - Setting select span width when showing
	bulletinInit: function() {
		var inputs = document.getElementsByTagName("select");
		for(a = 0; a < inputs.length; a++) {
			if(inputs[a].name.slice(0,8) == "bulletin") {
				var spanId = 'select'+inputs[a].name.slice(0,20);
				document.getElementById(spanId).style.width = inputs[a].offsetWidth - 23 + "px";
			}
		}
	...
	choose: function() {
		...
		if (this.name == "reoccur-end") enableDateInput();
	}
}
