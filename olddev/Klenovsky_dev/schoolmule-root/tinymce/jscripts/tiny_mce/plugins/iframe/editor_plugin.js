...
(function() {
	...
	tinymce.create('tinymce.plugins.iframePlugin', {
		...
		init : function(ed, url) {
			// Register the command so that it can be invoked by using tinyMCE.activeEditor.execCommand('mceiframe');
			ed.addCommand('mceiframe', function() {
				ed.windowManager.open({
					file : url + '/dialog.htm',
					width : 320 + parseInt(ed.getLang('iframe.delta_width', 0)),
					height : 120 + parseInt(ed.getLang('iframe.delta_height', 0)),
					inline : 1
				}, {
			...
			ed.onNodeChange.add(function(ed, cm, n) {
				cm.setActive('iframe', n.nodeName == 'IMG');
			});
			*/
		},
