		...
		init : function(ed, url) {
				...
				if (ed && ed.plugins.contextmenu) {
					ed.plugins.contextmenu.onContextMenu.add(function(plugin, menu, element) {
						if (element.nodeName === 'IMG' && element.className.indexOf('mceItemMedia') !== -1)
							menu.add({title : 'video.edit', icon : 'video', cmd : 'mceVideo'});
					});
				}
			});
			...
			ed.addCommand('mceVideo', function() {
				...
				ed.windowManager.open({
					file : url + '/video.htm',
					width : 430 + parseInt(ed.getLang('video.delta_width', 0)),
					height : 500 + parseInt(ed.getLang('video.delta_height', 0)),
					inline : 1
				}, {
					plugin_url : url,
					data : data
			...
			// Register buttons
			ed.addButton('video', {title : 'video.desc', cmd : 'mceVideo', image : url + '/img/video.png'});

			// Update media selection status
			ed.onNodeChange.add(function(ed, cm, node) {
				cm.setActive('video', isMediaImg(node));
			});
		},
