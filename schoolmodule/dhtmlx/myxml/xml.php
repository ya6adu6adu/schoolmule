...
if( $_REQUEST['action'] == 'create_xml' ){
	...
	*/
	...
	$dialog_grid = "<?xml version='1.0' encoding='UTF-8'?>
	<rows>
	";
	
	$tree_hidden = "<?xml version='1.0' encoding='UTF-8'?>
	<tree id=\"0\">
	...
	$tree_hidden .= "<item text=\"Quality column\" id=\"{$hide_quality}lev0_1_0_9 qualityBox 1\" />
		...
		<item text=\"Groups and objectives\" id=\"lev0_1_1\">";
			...
			*/
			...
			$res = mysql_query("SELECT * FROM `course_objectives` WHERE studygroup_id='{$studyGroupID}'")or die(mysql_error());
			...
			unset($sumWeight);
			$objectiveName = $_objectiveName->GetAll("","`studygroup_id`='{$studyGroupID}'");
				...
				*/
				$dialog_grid .= "
					<row id=\"row{$i}_{$n} {$objectiveName[$n]['objective_id']}\">
						<cell>{$objectiveName[$n]['title_en']}</cell>
						<cell>{$amount}%</cell>
					</row>";
