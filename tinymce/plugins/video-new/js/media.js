(function() {
	...
	window.Video = {
		...
		insert : function() {
			...
			editor.selection.setNode(editor.plugins.video.dataToImg(this.data));
			tinyMCEPopup.close();
		},

		preview : function() {
			get('prev').innerHTML = this.editor.plugins.video.dataToHtml(this.data, true);
		},

		moveStates : function(to_form, field) {
			var data = this.data, editor = this.editor, data = this.data,
				videoPlugin = editor.plugins.video, ext, src, typeInfo, defaultStates, src;
			...
			if (!to_form) {
				...
				if (field == 'src') {
					ext = src.replace(/^.*\.([^.]+)$/, '$1');
					if (typeInfo = videoPlugin.getType(ext))
		...
		formToData : function(field) {
			...
			if (field == 'source') {
				...
				setVal('source', this.editor.plugins.video.dataToHtml(this.data));
				this.panel = 'source';
			} else {
				if (this.panel == 'source') {
					this.data = this.editor.plugins.video.htmlToData(getVal('source'));
					this.dataToForm();
					this.panel = '';
				}
		...
		getMediaListHTML : function() {
			if (typeof(tinyMCEMediaList) != "undefined" && tinyMCEMediaList.length > 0) {
				...
				html += '<select id="linklist" name="linklist" style="width: 250px" onchange="this.form.src.value=this.options[this.selectedIndex].value;Video.formToData(\'src\');">';
